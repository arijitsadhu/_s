<?php
/**
 * jacky functions and definitions
 *
...
if ( ! defined( 'JACKY_VERSION' ) ) {
	// Replace the version number of the theme on each release.
	define( 'JACKY_VERSION', '1.0.0' );
}
...
if ( ! function_exists( 'jacky_setup' ) ) :
	...
	function jacky_setup() {
		 ...
		 * If you're building a theme based on jacky, use a find and replace
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'jacky' ),
			)
		);
...
function jacky_scripts() {
	wp_enqueue_style( 'jacky-style', get_stylesheet_uri(), array(), JACKY_VERSION );
	wp_style_add_data( 'jacky-style', 'rtl', 'replace' );

	wp_enqueue_script( 'jacky-navigation', get_template_directory_uri() . '/js/navigation.js', array(), JACKY_VERSION, true );

	wp_enqueue_script( 'jacky-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), JACKY_VERSION, true );
